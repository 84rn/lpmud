...
/*
 * Function name: set_money_tematy
 * Description:   Ustawia tematy nazw rodzajow kasy
 * Arguments:     a - The array of types (string)
 */
...
{
    int *c_arr, new_price, i, sum, *tmp_arr, max, tmp;
...
change_money(int *arr, object from, object to, int silent)
{
		 ...
		 arr[i], from, to)))
	{
	    ...
	    {
		if (!silent && (error2 < 0))
		    write("Wystapil blad przy pobieraniu pieniedzy od " +
		        "gracza. A moze w obiegu sa jakies falszywki?\n");
/*
...
*/
		 ...
		 arr[i], to, from)) && from)
	{
	    ...
	    {
		/* move error detected */
		write("Odkladasz czesc reszty.\n");
		saybb(QCIMIE(from, PL_MIA) + " odklada troche pieniedzy.\n");
		silent = 1;
	    }
...
text(int *arr, int przyp)
{
	...
	if (arr[i] > 0)
	{
	    ...
	    t_arr += ({ arr[i] + " " + 
	        oblicz_przym(MONEY_ODM[i][0], MONEY_ODM[i][1], prz, 
	            PL_ZENSKI, (arr[i] > 1)) });
	}
    ...
    int silent)
{
	...
	{
	    notify_fail("Udostepnij wieksza pule pieniedzy, gdyz " +
		text(arr, PL_MIA) + " to troche za malo, zeby " +
		" uiscic zadana przez sprzedawce kwote.\n");
	    return ({ 1 });
	}
	else  if (str && (str != ""))
	{
	    notify_fail("Wydaje mi sie, ze zdecydowal" + 
	        this_player()->koncowka("es", "as") + " sie zaplacic " +
		"pieniedzmi, ktorych nie masz przy sobie.\n");
	    return ({ 2 });
	}
	notify_fail("Nie starczy ci pieniedzy zeby zaplacic.\n");
	return ({ 3 });
    }
...
give(int price, object ob, string str, int test, object ob2, int silent)
{
	...
	ob = this_player();
    if (!ob && !test) {
	notify_fail("Nie ma komu dac pieniedzy.\n");
	return 0;
    }

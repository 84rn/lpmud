...
create()
{
    NoNews = ({
		...
		"This is unbelievable! No news to be found!?!\n",
		"No new news yet! If you have nothing else to do, you can write some yourself.\n",
		"Sorry, no news! Write some yourself so that others can be happy!\n",
		"WHAT HAVE YOU DONE??? All the news is DELETED!! Go redeem yourself!\n",
		"Since you haven't given another wizard a compliment today you won't be allowed to read all the new news!\n",
		"Argh! Dropped all the news into the sewer. Sorry.\n",
		"And so it was said: 'Thou shalt not read too much news...'\n",
		"The Newsboards are all stubborn today. They won't give you access to the news. Maybe a bribe will do?\n",
		...
		"Sorry, your favourite passtime is exhausted; No news.\n",
		});
...
restore_mbs()
{
    ...
    if (!m_sizeof(data))
    {
	write("You have no personal mbs save file, creating...\n");
	Selection = ORDER_GROUP;
	BdMap = ([]);
	SbMap = ([]);
	Groups = ({ "Boards" });

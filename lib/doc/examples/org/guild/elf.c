...
#define GUILD_NAME  "Elves secret club"
  /* Name of the file the shadow is defined in */
#define SHADOW      DIR + "elf_sh"

create_room()
{
    set_short("Elves guild");
    set_long("You are in the elves guild. You could try to join (jjjj)\n"+
	     "here. List and leave (llll) also works.\n");

    add_exit(DIR + "fighter", "east");
}
...
do_I_want_this_player_in_my_guild(player)
{
    ...
    {
	notify_fail("This club is only for elves.\n");
	return 0;
    }

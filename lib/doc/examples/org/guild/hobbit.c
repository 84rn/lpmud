...
#define GUILD_STYLE "funny"
  /* Hopefully a unique name of the guild. */
#define GUILD_NAME  "Hobbit popular"
  /* Name of the file the shadow is defined in */
#define SHADOW      DIR + "hobbit_sh"

create_room()
{
    set_short("Hobbits guild");
    set_long("You are in the hobbits guild. You could try to join (jjjj)\n"+
	     "here. List and leave (llll) also works.\n");

    add_exit(DIR + "fighter", "southeast");
}
...
do_I_want_this_player_in_my_guild(player)
{
    ...
    {
	notify_fail("This club is only for hobbits.\n");
	return 0;
    }

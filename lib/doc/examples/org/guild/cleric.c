...
#define GUILD_NAME  "Healers club"
  /* Name of the file the shadow is defined in */
#define SHADOW      DIR + "cleric_sh"

create_room()
{
    set_short("Clerics guild");
    set_long("You are in the clerics guild. You could try to join (jjjj)\n"+
	     "here. List and leave (llll) also works.\n");

    add_exit(DIR + "fighter", "west");
}
...
{
    if (this_player()->query_guild_style("thief"))
    {
	notify_fail("No thieves in our guild please.\n");
	return 0;
    }

...
#define GUILD_NAME  "The brave fighters guild"
  /* Name of the file the shadow is defined in */
#define SHADOW      DIR + "fighter_sh"

create_room()
{
    set_short("Fighters guild");
    set_long("You are in the fighters guild. You could try to join (jjjj)\n"+
	     "here. List and leave (llll) also works. Mages are north,\n" +
	     "thieves south, clerics east, elves west and hobbits\n" +
	     "northwest.\n");

    add_exit(DIR + "mage", "north");
    add_exit(DIR + "thief", "south");
    add_exit(DIR + "cleric", "east");
    add_exit(DIR + "elf", "west");
    add_exit(DIR + "hobbit", "northwest");
}
...
{
    if (this_player()->query_guild_style("magic"))
    {
	notify_fail("You are already a member of a mage guild, you " +
		    "cannot join us.\n");
	return 0;
    }

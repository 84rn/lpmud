/*
 * /std/living/tasks.c, written 920630 by Quis.
 ...
 *
 * Modified to allow VBFC in skill lists /Cmd 920707
 */
...
find_drm(mixed *skill_list)
{
    ...
    while(i < n) 
    {
        ...
        /* this added because the switch below can't take a function
         * pointer argument.
         */
        if (functionp(skill_list[i]))
        {
            mod = 2 * find_listval(skill_list[i++]);
        }
        else
        {
 	    switch (skill_list[i]) 
 ...
 */
public int
check_skill(int *skilllist, object player)
{
    int i, skill;

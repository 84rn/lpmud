...
DEFUN_VOID(initialize)
{
  ...
  if (_heapinfo == NULL)
    return 0;
  (void)memset(_heapinfo, 0, heapsize * sizeof(malloc_info));
...
DEFUN(morecore, (size), size_t size)
{
  ...
  if (BLOCK((char *) result + size) > heapsize)
      ...
      if (newinfo == NULL)
	{
      ...
      (void)memset(newinfo, 0, newsize * sizeof(malloc_info));
      (void)memcpy(newinfo, _heapinfo, heapsize * sizeof(malloc_info));
      oldinfo = _heapinfo;
...
DEFUN(realloc, (ptr, size), PTR ptr AND size_t size)
{
  ...
  switch (type)
      ...
      else
	{
	  ...
	  if (ptr != result)
	    (void)memmove(result, ptr, blocks * BLOCKSIZE);
	}
      break;
      ...
      else
	{
	  ...
	  if (result == NULL)
	    return NULL;
	  (void)memcpy(result, ptr, MIN(size, 1 << type));
	  free(ptr);
	}
      break;
    }
...
char *dump_malloc_data()
{
    ...
    mstat = mstats();
    (void)sprintf(buff,"\n\n\n"
    "Block type                Count                    Bytes\n\n"
    "allocated blocks:      %8d         %16d\n"
    "free blocks:           %8d         %16d\n"
    "-------------------    ---------       ------------------\n"
    "total blocks:          %8d         %16d (%d)\n\n",
    mstat.chunks_used, mstat.bytes_used,
    mstat.chunks_free, mstat.bytes_free,
    mstat.chunks_used + mstat.chunks_free,
    mstat.bytes_used + mstat.bytes_free,
    mstat.bytes_total);
    return buff;
}
